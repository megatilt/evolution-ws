...
/**
    ...
    Import: import { Apis } from "@evolution/chain"
...
class ApisInstance {
    ...
    connect( cs, connectTimeout ) {
        ...
        this.init_promise = this.ws_rpc.login(rpc_user, rpc_password).then(() => {
            ...
            this._db = new EvolutionApi(this.ws_rpc, "database");
            this._net = new EvolutionApi(this.ws_rpc, "network_broadcast");
            this._hist = new EvolutionApi(this.ws_rpc, "history");
            this._crypt = new EvolutionApi(this.ws_rpc, "crypto");
            var db_promise = this._db.init().then( ()=> {
                //https://github.com/cryptonomex/evolution/wiki/chain-locked-tx
